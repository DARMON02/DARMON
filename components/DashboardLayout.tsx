...
export const DashboardLayout: React.FC<Props> = ({ user, children }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200 font-sans relative">
      {/* Top Navigation */}
      ...
      {/* TEST MODE INDICATOR */}
      {ConfigService.TEST_MODE && (
        <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-1 opacity-90 pointer-events-none">
           <div className="bg-red-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg animate-pulse">
              TIME ACCEL ({ConfigService.TIME_SCALE}x)
           </div>
           {ConfigService.GPS_TEST_MODE && (
             <div className="bg-orange-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
                GPS TEST MODE (fake location)
             </div>
           )}
        </div>
      )}
